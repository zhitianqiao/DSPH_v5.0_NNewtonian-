...
void JSph::InitMultiPhase(const JXml *sxml,std::string xmlpath) {
  ...
  for(unsigned c=0; ele; c++) {
    sxml->CheckElementNames(ele,true,"rhop csound gamma visco tau_yield MC_phi Cohes tau_max Bi_multi HBP_n HBP_m cohesion phi phasetype");
  ...
  for(unsigned c=0; c<=PhaseCount-1; c++) {
	...
	if(PhaseCte[c].MC_phi)Log->Printf("  MC_phi: %f", ct.MC_phi);
