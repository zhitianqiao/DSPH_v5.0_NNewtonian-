...
//==============================================================================
__device__ void KerGetEta_Effective(const typecode ppx,float tau_yield, float pressp1, float MC_phi, float Cohes, float D_tensor_magn,float visco
  ,float m_NN,float n_NN,float &visco_etap1, float rhop)
{
	const float hudu_phi = MC_phi*(3.1415926) / 180;
	const float sin_phi = sin(hudu_phi);
	const float cos_phi = cos(hudu_phi);
	if (pressp1 < 0.f)pressp1 = 0; //avoid negative value
  if(D_tensor_magn<=ALMOSTZERO)D_tensor_magn=ALMOSTZERO;
  float miou_yield=(PHASECTE[ppx].tau_max ? (PHASECTE[ppx].tau_max) /(2.0f*D_tensor_magn) : (tau_yield+ fabs(pressp1) /rhop*sin_phi+Cohes*cos_phi)/(2.0f*D_tensor_magn)); //HPB will adjust eta		
  ...
  float visco_etap1_term1=(PHASECTE[ppx].tau_max ? miou_yield : (miouPap>m_NN*(tau_yield+ fabs(pressp1) / rhop *sin_phi + Cohes*cos_phi)||D_tensor_magn==ALMOSTZERO ? m_NN*(tau_yield+ fabs(pressp1) / rhop *sin_phi + Cohes*cos_phi) : miouPap));

  //HB
  float miouHB=visco*pow(D_tensor_magn,(n_NN-1.0f));
  float visco_etap1_term2=(bi_region ? visco : (miouPap>m_NN*(tau_yield+ fabs(pressp1) / rhop *sin_phi + Cohes*cos_phi)||D_tensor_magn==ALMOSTZERO ? visco : miouHB));
      ...
      if(compute) {
        ...
        if(tvisco==VISCO_Artificial) {//-Artificial viscosity.
        ...
        else if(tvisco==VISCO_LaminarSPS||tvisco==VISCO_ConstEq) {
          {
            ...
            KerGetEta_Effective(pp1,tau_yield,pressp1,MC_phi,Cohes,D_tensor_magn,visco_NN,m_NN,n_NN,visco_etap1, velrhop1.w);
